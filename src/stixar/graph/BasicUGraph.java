...
implements MutableUGraph
{
	 ...
	 */
	public BasicUGraph(final BasicUNode[] nodes)
	{
	 ...
	 */
	public BasicUGraph(final int n, final int m)
	{
		super(n, m);
	 ...
	 */
	@Override
	public BasicUNode node(final int id)
	{
		return nodes[id];
	}
	...
	public BasicUNode genNode()
	{
		...
		final BasicUNode n = new BasicUNode(this, nodeTop);
	...
	public void remove(final Node n)
	{
		...
		final BasicUNode bn = (BasicUNode) n;
		final CList<BasicUEdge> edges = new CList<BasicUEdge>();
		for(BasicUEdge e = bn.out(); e != null; e = e.next()) {
			edges.add(e);
		}
		for (final BasicUEdge e : edges) {
			remove(e);
		}
	...
	public void remove(final Edge e)
	{
		...
		final BasicUEdge be = (BasicUEdge) e;
		final int eid = be.edgeId();
		be.source().remove(be);
		eSlots.set(eid, false);
		final BasicUEdge rev = be.reverse();
		super.remove(be);
		rev.edgeId(-eid - 1);
		be.edgeId(-eid - 1);
		edgeCount--;
	 ...
	 */
	@Override
	public BasicUEdge genEdge(final Node u, final Node v)
	{
		...
		final BasicUNode bu = (BasicUNode) u;
		final BasicUNode bv = (BasicUNode) v;
		if (bu.ugraph() != this || bv.ugraph() != this) {
			throw new IllegalArgumentException();
		}
		final BasicUEdge res = BasicUEdge.getBasicUEdge(this, bu, bv, -1);
	...
	{
		final BasicUNode[] newNodes = new BasicUNode[newCap];
	...
	public Iterable<Node> nodes(final NodeOrder order)
	{
		...
		final ArrayList<Node> res = new ArrayList<Node>(nodeCount);
		if (order.isReversed()) {
	...
	protected void _shrinkEdges(final int newCap)
	{
		final int[] perm = super.shrinkEdges(newCap);
		eSlots.clear();
		for (int i=0; i<nodeTop; ++i) {
			final BasicUNode n = nodes[i];
			if (n == null) {
				continue;
			}
	...
	protected void _shrinkNodes(final int newCap)
	{
	...
	class BasicUNodeIterator implements Iterator<Node>
	{
		...
		BasicUNodeIterator(final BasicUNode[] nodes, final int nMods)
		{
			this.nodes = nodes;
		...
		public Node next()
		{
			...
			final BasicUNode result = nodes[index];
	...
	class BasicUEdgeIterator implements Iterator<Edge>
	{
		...
		BasicUEdgeIterator(final BasicUNode[] nodes, final int eMods)
		{
			this.nodes = nodes;
				...
				protected int nextNodeIndex(final int i)
		{
			...
			return res;
				...
				// edge e may be null.
				protected boolean trueEdge(final Edge e)
				{
          ...
          Edge: is any edge, or null.
				 */
				protected Edge nextEdge(final Edge e, int idx)
				{
					Edge res = e;
