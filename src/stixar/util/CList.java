...
implements List<T>, Queue<T>, Cloneable, Serializable
{
	 ...
	 * 
	 */
	private static final long serialVersionUID = -3784765948215704446L;
	 ...
	 */
	public CList(final CList<T> cList)
	{
		this();
		for (final T e : cList) {
			append(e);
	 ...
	 */
	public final ListCell<T> append(final T value)
	{
		numMods++;
		final Node<T> n = new Node<T>(this, value);
	 ...
	 */
	public final ListCell<T> prepend(final T value)
	{
		numMods++;
		final Node<T> n = new Node<T>(this, value);
	...
	public final T remove(final ListCell<T> cell)
	{
		final Node<T> node = (Node<T>) cell;
	 ...
	 */
	public final void append(final CList<T> list)
	{
		if (list == this) {
			throw new IllegalArgumentException
	...
	/**
       ...
       @throws IllegalArgumentException if called with this.append(this);
	 */
	public final void prepend(final CList<T> list)
	{
		if (list == this) {
			throw new IllegalArgumentException
	 ...
	 */
	public final void addFirst(final T elt)
	{
		prepend(elt);
	}
	 ...
	 */
	public final void addLast(final T elt)
	{
		append(elt);
	}
	...
	public final T removeFirst()
	{
		...
		numMods++;
		final T res = sentinel.next.value;
	...
	public final T removeLast()
	{
		...
		numMods++;
		final T res = sentinel.prev.value;
	 ...
	 */
	@Override
	public final boolean addAll(final int i, final Collection<? extends T> c)
	{
	...
	{
		final int i=0;
		final List<T> result = new CList<T>();
		for (Node<T> n = sentinel.next; n != sentinel; n = n.next) {
			if (i >= fromIndex) {
				if (i < toIndex) {
					result.add(n.value);
	 ...
	 */
	@Override
	public final boolean offer(final T o)
	{
		append(o);
		return true;
	}
	 ...
	 */
	@Override
	public final void add(final int i, final T v)
	{
		if (i < 0 || i >= size) {
			throw new IndexOutOfBoundsException();
		}
		numMods++;
		int j=0;
		final Node<T> node = new Node<T>(this, v);
		for (Node<T> n = sentinel.next; n != sentinel; n = n.next) {
			if (j++ == i) {
				node.prev = n.prev;
				node.next = n;
	 ...
	 */
	@Override
	public final T set(final int i, final T v)
	{
		if (i < 0 || i >= size) {
			throw new IndexOutOfBoundsException();
		}
	...
	public static <T> ListCell<T> nullCell()
	{
		return new ListCell<T>() {
			...
			@Override
			public T value(final T v) { return null; }
			@Override
	...
	protected static class Node<T> implements ListCell<T>
	{
		...
		Node(final CList<T> list, final T value)
		{
			this.value = value;
			this.next = null;
			this.prev = null;
		...
		@Override
		public final T value(final T v)
		{
			return value = v;
		}
	...
	implements ListIterator<T>
	{
		...
		public NodeIterator(final Node<T> node, final CList<T> list)
		{
			this.node = node;
			this.pos = 0;
			this.forward = true;
		...
		@Override
		public final void set(final T v)
		{
		...
		public final void add(final T v)
		{
			...
			expectedMods++;
			node.list.numMods++;
			final Node<T> n = new Node<T>(node.list, v);
			n.prev = node.prev;
			node.prev.next = n;
			n.next = node;
			node.prev = n;
